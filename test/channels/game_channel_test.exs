defmodule Pokerboy.GameChannelTest do
  ...
  describe "playing game" do
    ...
    test "max username size of 10", %{socket: socket} do
    ...
    test "it can toggle playing", %{socket: socket} do
    ...
    test "it can kick player", %{socket: socket} do
      {_, socket: _} = join_channel(socket.assigns.game_id, %{"name" => "lucas2"})
      assert_push "game_update", %{status: :ok}
      assert_push "game_update", %{status: :ok}
    ...
    test "it can vote", %{socket: socket} do
      push socket, "user_vote", %{"vote" => "5"} 
      assert_push "game_update", %{status: :ok}
    end  

    test "it can store original vote", %{socket: socket} do
    ...
    test "it rejects invalid vote", %{socket: socket} do
      push socket, "user_vote", %{"vote" => ":)"} 
      assert_push "game_update", %{status: :error}
    end  

    test "it does not show user votes", %{socket: socket} do
      {_, socket: _} = join_channel(socket.assigns.game_id, %{"name" => "lucas2"})
    ...
    test "it does show user votes on reveal", %{socket: socket} do   
      push socket, "toggle_playing", %{"user" => "lucas"}
      assert_push "game_update", %{status: :ok}         
      push socket, "user_vote", %{"vote" => "5"} 
      assert_push "game_update", %{status: :ok, state: state}
    ...
    test "it can reveal", %{socket: socket} do
      push socket, "toggle_playing", %{"user" => "lucas"}
      assert_push "game_update", %{status: :ok}
    ...
    test "it does not reveal on no players", %{socket: socket} do
      push socket, "toggle_playing", %{"user" => "lucas"}    
      assert_push "game_update", %{status: :ok, state: state}
    ...
    test "it can force reveal", %{socket: socket} do
      push socket, "reveal", %{} 
      assert_push "game_update", %{status: :ok, state: state}
    ...
    test "it can reset", %{socket: socket} do
      push socket, "user_vote", %{"vote" => "5"} 
      assert_push "game_update", %{status: :ok}
    ...
    test "it leaves on disconnect", %{socket: socket} do
    ...
    test "it has valid votes", %{socket: socket} do   
